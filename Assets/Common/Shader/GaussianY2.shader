	...
	SubShader {
		...
		Pass {
			...
			float4 frag(vs2ps i) : COLOR {
				...
				float4 cd = tex2D(_MainTex, i.uv + float2(0.0, -4.0 * dx.y));
				float4 cc = tex2D(_MainTex, i.uv + float2(0.0, -3.0 * dx.y));
				...
				float4 c3 = tex2D(_MainTex, i.uv + float2(0.0,  3.0 * dx.y));
				float4 c4 = tex2D(_MainTex, i.uv + float2(0.0,  4.0 * dx.y));
